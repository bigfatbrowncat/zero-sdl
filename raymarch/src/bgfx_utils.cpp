...
static bx::FileReaderI* myFileReader = new bx::CrtFileReader();
static bx::AllocatorI* myAllocator = new bx::CrtAllocator();
...
/*bgfx::TextureHandle loadTexture(bx::FileReaderI* _reader, const char* _name, uint32_t _flags, uint8_t _skip, bgfx::TextureInfo* _info)
{
	...
	bgfx::TextureHandle handle = BGFX_INVALID_HANDLE;
	bx::AllocatorI* allocator = myAllocator;
...
{
	bx::FileReaderI* reader = myFileReader;
...
{
	MeshState* state = (MeshState*)BX_ALLOC(myAllocator, sizeof(MeshState) );
	return state;
}

void meshStateDestroy(MeshState* _meshState)
{
	BX_FREE(myAllocator, _meshState);
}
